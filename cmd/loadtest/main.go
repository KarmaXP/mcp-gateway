...
func oneIteration(client *http.Client, base, mode string, cfg callConfig) (callLatency time.Duration, err error) {
	...
	conn, err := mcphostclient.Dial(ctx, client, base, cfg.bearer)
	if err != nil {
		return 0, err
	}
	defer conn.Close()

	if _, err := conn.Call(ctx, mcpwire.MethodInitialize, map[string]any{
		"protocolVersion": mcpwire.MCPProtocolVersion,
		"capabilities":    map[string]any{},
		"clientInfo":      map[string]any{"name": "loadtest", "version": "0"},
	}, loadtestInitAckTimeout); err != nil {
		return 0, err
	}
	if err := conn.Notify(ctx, "notifications/initialized", nil); err != nil {
		return 0, err
	}
	if _, err := conn.Call(ctx, mcpwire.MethodToolsList, nil, loadtestToolsListTimeout); err != nil {
		return 0, err
	}

	toolName, callArgs := cfg.directTool, cfg.directArgs
	if mode == "semantic" {
		toolName, callArgs = cfg.semanticTool, "{}"
	}
	var arguments any
	if err := json.Unmarshal([]byte(callArgs), &arguments); err != nil {
		return 0, fmt.Errorf("tool arguments: %w", err)
	}

	t0 := time.Now()
	raw, err := conn.Call(ctx, mcpwire.MethodToolsCall, map[string]any{
		"name":      toolName,
		"arguments": arguments,
	}, loadtestToolsCallTimeout)
	if err != nil {
		return 0, err
	}
