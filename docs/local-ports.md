...
| Key | Value | Use |
|-----|-------|-----|
| Dev gateway port | **8080** (override in `.env`, e.g. `PORT=18080` for multibackend benchmark) | `PORT` / `GATEWAY_PORT`; `make run` and `make stop` both source `.env` |
...
| JWT multibackend benchmark smoke | **18082** | Optional `GATEWAY_URL` in JWT smoke walkthroughs (see [DEVELOPER.md](DEVELOPER.md)) |
