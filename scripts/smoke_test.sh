...
if [[ "${DEMO_PRINT_HELP:-}" == "1" ]]; then
  ...
  echo ""
  echo "Driving it by hand. Every reply arrives on the SSE stream; the POST only returns 202."
  echo ""
  echo "  1. In another terminal, open the stream and keep it open. Note the Mcp-Session-Id header:"
  echo "       curl -N -D - ${GATEWAY_URL}/mcp/sse"
  echo ""
  echo "  2. Here, complete the handshake before calling a tool. Without both steps the gateway"
  echo "     answers -32001 handshake incomplete:"
  echo "       SID=<the Mcp-Session-Id from step 1>"
  echo "       post() { curl -s -o /dev/null -w '%{http_code}\\n' -H \"Mcp-Session-Id: \${SID}\" \\"
  echo "                -H 'Content-Type: application/json' -d \"\$1\" ${GATEWAY_URL}/mcp/rpc; }"
  echo "       post '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{\"protocolVersion\":\"2024-11-05\",\"capabilities\":{},\"clientInfo\":{\"name\":\"curl\",\"version\":\"0\"}}}'"
  echo "       post '{\"jsonrpc\":\"2.0\",\"method\":\"notifications/initialized\"}'"
  echo "       post '{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"smoke__echo\",\"arguments\":{}}}'"
  echo ""
  echo "  3. The results appear in the terminal from step 1. Full handshake: docs/CONNECTING_AGENTS.md"
